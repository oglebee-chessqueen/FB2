		...
		if (val == FILE_MODE)
		{
			while (1)
			{
				...
				else
				{
					struct stat finfo;
					stat(filename, &finfo);
					Params->bytesRemaining = finfo.st_size;
					FILE *fp = fopen("txlog.txt", "a");
					fprintf(fp, "Filesize: %d\n", Params->bytesRemaining);
					fclose(fp);
					break;
				}
			}
...
void* thread_read(void* arg)
{
	...
	unsigned int bytesRemaining;  // Sent from TX frame header
	unsigned int framesize = sizeof bytesRemaining + 1;
...
Begin:
	...
	while (1)
	{
		...
		// If this is the first byte of the packet, read frame header
		// 0x3A is an arbitrary magic byte as an extra check for first-byte
		if (curNumbytes == 0)
		{
			if (pBuf[0] != 0x3A)
				printf("Frame header mismatch!");
			// Gather next 4 bytes as int, skipping the magic byte
			memcpy(&bytesRemaining, &pBuf[1], sizeof bytesRemaining);
			// Swap endianness :(
			//~ bytesRemaining =
				//~ (bytesRemaining & 0x000000ff) << 24u |
				//~ (bytesRemaining & 0x0000ff00) << 8u |
				//~ (bytesRemaining & 0x00ff0000) >> 8u |
				//~ (bytesRemaining & 0xff000000) >> 24u
			//~ ;
		}
		curNumbytes += len;
		...
		fp = fopen("rxlog.txt", "a");
		fprintf(fp, "Framesize: %d, len: %d, ", framesize, len);
		fprintf(fp, "Bytes remaining: %d => 0x", bytesRemaining);
		for (i = framesize; i > 0; i--)
			fprintf(fp, "%02x", pBuf[i]);
		fprintf(fp, "\n", pBuf[i]);
		fclose(fp);
		// If no more bytes to receive, quit
		if (bytesRemaining <= len)
		{
			port_args->mode.rx = 0; // To break from main() loop
			break;
		}
	...
	while (1)
	{
		...
		if (port_args->mode.tx == FILE_MODE)
		{
			while (!feof(port_args->fp))
			{
				...
				// Frame header containing arbitrary magic byte 0x3A + four(?) bytes
				//   containing the total number of remaining bytes to trasmit
				buf[0] = 0x3A;
				framesize =  sizeof port_args->bytesRemaining + 1;
				fp = fopen("txlog.txt", "a");
				fprintf(fp, "Framesize: %d, ", framesize);
				fclose(fp);
				memcpy(&buf[1], &port_args->bytesRemaining, framesize - 1);
				fp = fopen("txlog.txt", "a");
				fprintf(fp, "memcpy complete: %d => 0x", port_args->bytesRemaining);
				for (i = 5; i > 0; i--) // next 4 bytes big endian
					fprintf(fp, "%02x", buf[i]);
				// Read packetSize-framesize bytes into buffer
				ret = fread(&buf[framesize], 1, port_args->packetSize - framesize, port_args->fp);
				// Write buffer to comsync device
				ret = write(port_args->fd, &buf, ret);
				fprintf(fp, ", write ret: %d\n", ret);
				fclose(fp);
				bytes += ret;
				port_args->bytesRemaining -= ret;
