	...
	else if(mode == RX)
	{
		...
		if(val == FILE_MODE)
		{
			while(1)
			{
				...
				scanf("%s",filename);
				Params->fp = fopen(filename,"wb");	  // Opens file to write received bytes to
				if (!Params->fp)
				{
	...
	else if(mode == TX)
	{
		...
		if(val == FILE_MODE)
		{
			while(1)
			{
				printf("enter file name\n");
				scanf("%s",filename);
				Params->fp = fopen(filename,"rb");		// Opens file to read transmitted data
				if (!Params->fp)
				{
					printf("invalid filename\n");
					continue;
...
void *thread_read(void *arg) {
  ...
  int packetSize;
	//create a timeout variable; should prevent the executable from
	//timing out, especially for larger files
  ...
  while(1)
	{
		...
		{
			tmpfds = readfds;
			i = select(fdwidth, &tmpfds, NULL, NULL, &timeout);
			printf("select %d\n", i);
			if(i <= 0) {
