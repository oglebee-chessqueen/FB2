...
def read_fits(filename):
	...
	try:
		...
		"File %s: CORRUPT -- skipping" % (filename)
		try:
			hdulist = fits.open(filename, ignore_missing_end=True)
			print "File OPENED with IGNORE_MISSING_END"
		except:
			pass
		returncode = 1
		return returncode, 0
...
directory = "C:\Users\Keri Hoadley\Documents\FIREBall-2\python\\1MBit_test\\"
filename_root = "image" #Substring to find in filenames
num0 = 99 #File number you want to start on
print num0
...
# Field 5 - 8: 5 = Long Slit, 6 = Diagonal Slit; 7 = GRID; 8 = Tilted Mask
field = 3
...
filename = [
	'test.fits',
	#~ 'image_170923_000000.fits',
	'image_170923_000001.fits',
	'image_170923_000002.fits',
	'image_170923_000003.fits',
	'image_170923_000004.fits',
	#~ 'test_cont1.fits',
	#~ 'test_cont2.fits',
	#~ 'test_cont3.fits',
	#~ 'test_tx1_1.fits',
	...
	#~ 'test_rx1_2.fits',
	#~ 'test_rx1_3.fits',
	#~ 'test1_txb_1.fits',	# DON'T OPEN
	#~ 'test1_txb_2.fits',	# DON'T OPEN
	#~ 'test1_txb_3.fits',	# DON'T OPEN
	#~ 'test1_rxb_1.fits',	# DON'T OPEN
	#~ 'test1_rxb_2.fits',
	#~ 'test1_rxb_3.fits',
	#~ 'test1_txb_1.fits',	# DON'T OPEN
	#~ 'test1_txb_2.fits',	# DON'T OPEN
	#~ 'test1_txb_3.fits',	# DON'T OPEN
	#~ 'test1_txrxb_1.fits',	# DON'T OPEN
	#~ 'test1_txrxb_2.fits',	# DON'T OPEN
	#~ 'test1_txrxb_3.fits',	# DON'T OPEN
]
...
while not(quit):
	for file in range(0,len(filename)):
		...
		for f in os.listdir(directory):
			if filename[file] in f: filefound=True
		...
		# IF fits file found with correct name, then open and do operations
			...
			# Read in image file
			returncode, data = read_fits(os.path.join(directory,filename[file]))
			if returncode == 1:
				print "File cannot be opened; continue"
				continue
			...
			ax1.set_ylim(0,numpy.max(y_range2))
			...
			#~ plt.show(block=False)
			plt.show()
			#~ plt.savefig(os.path.join(directory,filename[file]+'.png'),orientation='landscape')
			plt.clf()
			#~ time.sleep(delay)
			#~ print 'End of sleep'
			#~ num+=1
