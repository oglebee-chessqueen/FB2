...
def main():
	...
	while True:
		...
		if filefound:
			...
			returncode = comm_command(program, filepath)		#new_filepath)
			if returncode != 0:
				log(logfile, "Error executing command: %s %s" % (program, filepath)		#new_filepath)new_filepath))
				break
			else:
				log(logfile, "%s successfully sent!" % (filepath)		#new_filepath)
...
######## NOT NEEDED FOR THIS PROGRAM - USED IN fb2_rx.py ########
