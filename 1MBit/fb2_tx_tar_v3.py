...
def main():
	...
	while True:
		...
		# 1. Open Tx port and purge the line
		# Receive tar file using data transfer C code
		try:
			# 1. Tx Purge
			# Try to purge Tx before transmitting
			try:
				# NEED THIS TO OPEN HERE
				...
				print "Tx purge complete"
				log(logfile,"Tx purge: SUCCESS")
			except:
				print "Tx purge didn't work?"
				log(logfile,"Tx purge: FAILED")
				break

			# Leave line open while python looks for next image file
			...
			if filefound:
				...
				size_delay = ((filesize*8/1e6)+((filesize/packet)*0.001))#/3
				time.sleep(delay)

				# 2. Send TAR file (outfile) through Tx
				#    Close the Tx line afterwards.
				try:
					print "Sending tarred folder with image %06i over Tx: file size = %i bytes"%(filenum,filesize)
					print "Total time expected for transfer: %i seconds"%(size_delay)
					p.stdin.write("11\n %i\n 1\n 2\n %s\n y\n" % (packet,outfile))
					time.sleep(size_delay)
					print "file transfer complete"
					log(logfile,"file transfer complete: Tx file size = %i bytes for image %06i"%(filesize,filenum))
					print "killing Tx line in %i seconds"%(delay)
					time.sleep(delay)
					os.kill(pid, 9)
