...
def main():
	...
	packet = 256	#512		# packet size to use, up to 4096
	...
	while True:
		...
		filepath = os.path.join(directories["img"]+'180601', filename)#datetime.now().strftime("%y%m%d"), filename)
		#print filepath
		...
		if filefound:
			...
			returncode = tar_files(filename, housekeeping, directories["img"]+"180601", directories["hk"]+datetime.now().strftime("%y%m%d"), outfile, logfile)	# All files in this compression file!!
			#time.sleep(20)
			...
			print "waiting %i seconds for Rx to start/be ready"%(delay)
			time.sleep(delay)
			...
			log(logfile, "Waiting %d for checking next file..." % (size_delay/3))
...
def tar_files(img, hk, dir_img, dir_hk, compression, logfile):
	'''
	img and hk files already have directories attached to them
	img and hk should not have directories associated with them when they are tarred
	'''
	p=subprocess.call(["tar","-cvf","temp.tar","-C",dir_img,img])
	for i in hk:
		if os.name == "posix":		# linux: use Popen for command line tarball
			#tar = compression
			try:
				p=subprocess.call(["tar","-vrf",'temp.tar',"-C",dir_hk,i])
	...
	p=subprocess.call(["tar","-cvzf",compression,"temp.tar"])
