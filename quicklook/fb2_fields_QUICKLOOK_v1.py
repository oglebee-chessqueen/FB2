...
def main():
	...
	filename_root_image = "image_170924_"	#"image" #Substring to find in filenames
	...
	#~ num0 = 0			# IMAGE File number you want to start on
	#~ field = 1
	picklefile = os.path.join(root,'fb2_fields_v1.pickle')
	...
	while not(quit):
		...
		plotfile = "spectra_%s%02i" % (filename_root_event,field)
		...
		# IF fits file found with correct name, then open and do operations
			...
			data = read_fits(os.path.join(directories["img"],filename))
			...
			file1 = os.path.join(directories["img"],file1)
			file2 = "events_field%02i_170924.fits" % (field)
			file2 = os.path.join(directories["img"],file2)
			...
			#~ plt.show()
			plt.savefig(os.path.join(directories["img"],filename+'.png'),orientation='landscape')
			plt.clf()
