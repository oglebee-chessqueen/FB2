...
def plot_spectra(data,x_spec,x_end,y_spec,y_wid,wavelength,file):
	...
	for i in range(0,len(x_spec)):
		...
		plt.text(194,numpy.max(y)-5,str(i+1),weight='bold',color='r')
		if j == 10 or j == 11:
		...
		if (j == 11) or (i == len(x_spec)-1):
			plt.savefig("%s_%02i%s" % (file,fnum,prefix), format='eps', dpi=900)
			fnum += 1
			plt.clf()
		else:
			continue
	...
	for i in range(0,numpy.size(extract_range)-1):
		...
		if numpy.size(peakind) != 0:		# Test to see if there are any elements left in peakind
			...
			for ii in peakind:
				...
				if (abs(parms[2]) >= 3.5) and (abs(parms[2]) < 10):
...
picklefile = 'fb2_fields_v1.pickle'
...
while not(quit):
	filename = "%s%02i_old.fits" % (filename_root,num)			# If events_fieldsxx.fits
	...
	# IF fits file found with correct name, then open and do operations
		...
		plt.savefig(plotfile+'.eps', dpi=900, format='eps')#, orientation='landscape')
